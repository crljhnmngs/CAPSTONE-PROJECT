...
<head>
  ...
  <title>Client - View vehicles</title>
...
<?php  include('D:\PROGRAMMING SOFTWARES\XAMPP\htdocs\WeLift\includes\renterheader.php');      ?>
    ...
    <?php
      ...
      <?php if($result->accountstatus == 'VERIFIED') { ?>
        ...
        <?php
               ...
               $sql="SELECT COUNT(*) AS total FROM vehicle where status = 'AVAILABLE' OR status = 'PENDING'";
               $query=$dbh->prepare($sql);
               $query->execute();
               $results=$query->fetchALL(PDO::FETCH_OBJ);
        ...
        <div class="col-xs-12" >
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"><i class="fa fa-cab"></i> Available Vehicles </h3>
            </div>
           ...
           <label class="box-title" style="margin-top:10px;"> Filter by vehicle type: </label>
            ...
            <option value="" disabled selected="" hidden >Select Filter</option>
              ...
              <div>
              <a href="viewvehicles.php"><button style="margin-bottom:5px;width:150px; "  type="sumbit" name="clear" class="btn btn-primary px-4">Clear Filter</button></a>
                </div>
                 <!-- /.Filter -->
          ...
          <div class="searchbox" style="display:inline-block;float:right;">
             <input style="width:270px;" type="text" name="search" value="<?php if(isset($_GET['search'])){echo $_GET['search'];} ?>"  placeholder="Search by Vehicle Model or Brand">
          </div>
          </form>       
          </div>
              ...
              <?php
                   if(isset($_GET['start_price']) && isset($_GET['end_price']) && isset($_GET['selector'])){
                    ...
                    WHERE vehicle.status = 'AVAILABLE' AND user.accountstatus != 'DEACTIVATED' AND  vehicle.rate  BETWEEN $startprice AND $endprice AND vehicle.vehicletype = '".$selector."'";
                    $query=$dbh->prepare($sql);
                    $query->execute();
                    $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
                ...
                $sql="SELECT * FROM vehicle 
                INNER JOIN user ON user.userid=vehicle.userid
                 WHERE vehicle.status = 'AVAILABLE' AND user.accountstatus != 'DEACTIVATED' AND vehicle.vehicletype = '".$selector."'";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                ...
                $sql="SELECT * FROM vehicle 
                INNER JOIN user ON user.userid=vehicle.userid
                 WHERE  (vehicle.model LIKE '%$search%' OR vehicle.brand LIKE '%$search%') AND vehicle.status = 'AVAILABLE' AND user.accountstatus != 'DEACTIVATED'";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                }
                 ...
                 $sql="SELECT * FROM vehicle 
                 INNER JOIN user ON user.userid=vehicle.userid
                 WHERE vehicle.status = 'AVAILABLE' AND vehicle.rate  BETWEEN $startprice AND $endprice AND user.accountstatus != 'DEACTIVATED'";
                 $query=$dbh->prepare($sql);
                 $query->execute();
                 $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
                ...
                else
                {
                    $sql="SELECT * FROM vehicle 
                    INNER JOIN user ON user.userid=vehicle.userid
                    WHERE (vehicle.status = 'AVAILABLE' OR vehicle.status = 'PENDING') AND user.accountstatus != 'DEACTIVATED'";
                     $query=$dbh->prepare($sql);
                     $query->execute();
                     $results=$query->fetchALL(PDO::FETCH_OBJ);
                }
 ...
 <!--cards -->
    ...
    <div class="card">
        <div class="image2">
        <a href="#edit_<?php echo htmlentities($result->vehicleid);?>" data-toggle="modal">       
          <img  src="../owner/vehicle_images/<?php echo htmlentities($result->vehicleimage);?>">
          </a>
    </div>
    <div class="title"> 
...
<!--cards -->
      ...
      <div class="row" >
        <div class="col-xs-12" >
          ...
          <div class="searchbox" style="display:inline-block;float:right;">
            ...
            <h4 style="margin-top:3%;text-align:center">  You need to <bold>Verify<bold> your account to book a vehicle! </h4>
            <h4 style="margin-bottom:-3%;text-align:center">  Go to your <a href="profile.php">Profile</a> page and Upload Valid ID's. </h4>
              ...
              <?php
                ...
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                  ...
                  if($startprice != null && $endprice == null )
                  {
                    echo '<script>alert("Please input End Price!")</script>'; 
                    echo "<script type ='text/javascript'> document.location.href='viewvehicles.php' </script>"; 
                  }
                  else if($endprice != null && $startprice == null )
                  {
                ...
                else
                {
                    $sql="SELECT * FROM vehicle 
                    INNER JOIN user ON user.userid=vehicle.userid
                    WHERE vehicle.status = 'AVAILABLE' AND user.accountstatus != 'DEACTIVATED'";
                     $query=$dbh->prepare($sql);
                     $query->execute();
                     $results=$query->fetchALL(PDO::FETCH_OBJ);
                }
 ...
 <!--cards -->
    ...
    <?php include('notverifiedinfo.php'); ?>             
<!--cards -->
  <?php } 
    } ?>
     </div>
      ...
      <div class="row" >
        <div class="col-xs-12" >
          <div class="box">
            ...
            <h4 style="margin-top:1%;text-align:center">  You need to be <bold>Verified<bold> first to book a vehicle! </h4>
            <h4 style="margin-bottom:-3%;text-align:center">  Go to your <a href="profile.php">Profile</a> page and Upload Valid ID's. </h4>
