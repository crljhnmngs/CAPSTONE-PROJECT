...
if(!isset($_SESSION['userid']))
  {
    ...
    //$currentdatetime = strtotime(date('2022-05-25 00:36:00'));
...
<head>
  ...
  <title>Client - Vehicle Bookings</title>
...
<?php  include('D:\PROGRAMMING SOFTWARES\XAMPP\htdocs\WeLift\includes\renterheader.php');      ?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1 align="center">
        <i class="fa fa-car"></i> Vehicle Bookings
        <small></small>
      </h1>
    ...
    <?php
               ...
               WHERE booking.renterid = $id AND bookingstatus = 'PENDING'";
               $query=$dbh->prepare($sql);
               $query->execute();
               $results=$query->fetchALL(PDO::FETCH_OBJ);
          ...
          <div class="hide1" style="float:right;margin-top:80px;">Vehicle Booking will automatically cancelled after 6 hours.</div>
              ...
              <?php
                   if(isset($_GET['start_price']) && isset($_GET['end_price']) && isset($_GET['selector'])){
                    ...
                    INNER JOIN vehicle ON booking.vehicleid = vehicle.vehicleid
                    INNER JOIN user ON vehicle.userid = user.userid
                    WHERE booking.renterid = $id AND bookingstatus = 'PENDING' AND  vehicle.rate  BETWEEN $startprice AND $endprice AND vehicle.vehicletype = '".$selector."'";
                    $query=$dbh->prepare($sql);
                    $query->execute();
                    $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
                ...
                $sql="SELECT * FROM booking
                    INNER JOIN vehicle ON booking.vehicleid = vehicle.vehicleid
                    INNER JOIN user ON vehicle.userid = user.userid
                    WHERE booking.renterid = $id AND bookingstatus = 'PENDING' AND vehicle.vehicletype = '".$selector."'";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                ...
                if($search == null)
                {
                  ...
                  INNER JOIN vehicle ON booking.vehicleid = vehicle.vehicleid
                  INNER JOIN user ON vehicle.userid = user.userid
                  WHERE booking.renterid = $id AND (vehicle.model LIKE '%$search%' OR vehicle.brand LIKE '%$search%') AND bookingstatus = 'PENDING' ";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                }
                  ...
                  else if($endprice < $startprice )
                  {
                    ...
                    INNER JOIN vehicle ON booking.vehicleid = vehicle.vehicleid
                    INNER JOIN user ON vehicle.userid = user.userid
                    WHERE booking.renterid = $id AND bookingstatus = 'PENDING' AND vehicle.rate  BETWEEN $startprice AND $endprice ";
                 $query=$dbh->prepare($sql);
                 $query->execute();
                 $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
