...
<head>
  ...
  <title>Client - Reserved Vehicles</title>
...
<?php  include('D:\PROGRAMMING SOFTWARES\XAMPP\htdocs\WeLift\includes\renterheader.php');      ?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1 align="center">
        <i class="fa fa-car"></i> Reserved Vehicles
        <small></small>
      </h1>
    ...
    <?php
               ...
               where rentercredentials.renterid = $id AND (booking.bookingstatus = 'RESERVED' OR booking.bookingstatus = 'TO_CONFIRM')";
               $query=$dbh->prepare($sql);
               $query->execute();
               $results=$query->fetchALL(PDO::FETCH_OBJ);
        ...
        <div class="col-xs-12" >
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"><i class="fa fa-cab"></i> Reserved Vehicles </h3>
            </div>
           ...
           <label class="box-title" style="margin-top:10px;"> Filter by vehicle type: </label>
            ...
            <option value="" disabled selected="" hidden >Select Filter</option>
              ...
              <div>
              <a href="rentedvehicles.php"><button style="margin-bottom:5px;width:150px; "  type="sumbit" name="clear" class="btn btn-primary px-4">Clear Filter</button></a>
                </div>
                 <!-- /.Filter -->
             ...
             <input style="width:270px;" type="text" name="search" value="<?php if(isset($_GET['search'])){echo $_GET['search'];} ?>"  placeholder="Search by Vehicle Model or Brand">
              ...
              <?php
                   if(isset($_GET['start_price']) && isset($_GET['end_price']) && isset($_GET['selector'])){
                    ...
                    INNER JOIN vehicle ON booking.vehicleid = vehicle.vehicleid
                    INNER JOIN user ON vehicle.userid = user.userid
                    WHERE booking.renterid = $id AND vehicle.rate  BETWEEN $startprice AND $endprice AND vehicle.vehicletype = '".$selector."' AND (booking.bookingstatus = 'RESERVED' OR booking.bookingstatus = 'TO_CONFIRM')";
                    $query=$dbh->prepare($sql);
                    $query->execute();
                    $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
                ...
                $sql="SELECT * FROM booking
                    INNER JOIN vehicle ON booking.vehicleid = vehicle.vehicleid
                    INNER JOIN user ON vehicle.userid = user.userid
                    WHERE booking.renterid = $id AND vehicle.vehicletype = '".$selector."' AND (booking.bookingstatus = 'RESERVED' OR booking.bookingstatus = 'TO_CONFIRM')";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                ...
                if($search == null)
                {
                  ...
                  INNER JOIN vehicle ON booking.vehicleid = vehicle.vehicleid
                  INNER JOIN user ON vehicle.userid = user.userid
                  WHERE booking.renterid = $id AND (vehicle.model LIKE '%$search%' OR vehicle.brand LIKE '%$search%') AND (booking.bookingstatus = 'RESERVED' OR booking.bookingstatus = 'TO_CONFIRM') ";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                }
                  ...
                  else if($endprice != null && $startprice == null )
                  {
                    ...
                    echo "<script type ='text/javascript'> document.location.href='vehiclerental.phpp' </script>"; 
                  }
                  else if($endprice == null && $startprice == null )
                  {
                  ...
                  else if($endprice < $startprice )
                  {
                    ...
                    INNER JOIN vehicle ON booking.vehicleid = vehicle.vehicleid
                    INNER JOIN user ON vehicle.userid = user.userid
                    WHERE booking.renterid = $id AND vehicle.rate  BETWEEN $startprice AND $endprice AND (booking.bookingstatus = 'RESERVED' OR booking.bookingstatus = 'TO_CONFIRM')";
                 $query=$dbh->prepare($sql);
                 $query->execute();
                 $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
                ...
                else
                {
                  ...
                  INNER JOIN vehicle ON booking.vehicleid = vehicle.vehicleid
                  INNER JOIN user ON vehicle.userid = user.userid
                  where booking.renterid = $id AND (booking.bookingstatus = 'RESERVED' OR booking.bookingstatus = 'TO_CONFIRM')";
                     $query=$dbh->prepare($sql);
                     $query->execute();
                     $results=$query->fetchALL(PDO::FETCH_OBJ);
                }
     ...
     ?>
      ...
      <div class="row" >
        <div class="col-xs-12" >
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"><i class="fa fa-cab"></i> Vehicle Rentals </h3>
