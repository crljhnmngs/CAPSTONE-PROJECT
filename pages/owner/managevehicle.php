...
if(isset($_POST['delete'])){
    ...
    $image = $_POST['image'];
    $query="DELETE FROM vehicle where vehicleid=$deleteid";
    $query1 = $dbh->prepare($query);
    $query1->execute();
    unlink("vehicle_images/".$image);
...
<head>
  ...
  <title>Vehicle Owner - Manage Vehicles</title>
  ...
  <link rel="stylesheet" href="../../dist/css/ownermanagevehicles.css">
...
<?php  include('D:\PROGRAMMING SOFTWARES\XAMPP\htdocs\WeLift\includes\ownerheader.php');      ?>
  ...
  <div class="content-wrapper">
    ...
    <?php
              ...
              ?>
      <?php if($result->accountstatus == 'VERIFIED') { ?>


        <?php
              $sql="SELECT COUNT(*) AS total FROM vehicle where userid = $id AND status != 'DELETED'";
               $query=$dbh->prepare($sql);
               $query->execute();
               $results=$query->fetchALL(PDO::FETCH_OBJ);
              ...
              ?>
    <!-- Main content -->
    <section class="content" style="height=50px;">
      <div class="row" >
        <div class="col-xs-12" >
          <div class="box" style="min-height: 75vh;" >
            <div class="box-header">
              <h3 class="box-title"><i class="fa fa-cab"></i> Manage Vehicle</h3><a href="addvehicle.php"><button type="button" class="btn btn-success btn-sm" style="margin-left: 2%" >
               <i class="fa fa-plus" style="margin"></i> Add
              </button></a>
           ...
           <label class="box-title" style="margin-top:10px;"> Filter by vehicle type: </label>
            ...
            <option value="" disabled selected="" hidden >Select Filter</option>
              ...
              <label class="box-title" style="margin-top:10px;"> Filter by vehicle status: </label>
            <select style="width:200px;"  class="form-control" name="status" value="<?php if(isset($_GET['status']))?>" >
                    <option selected>AVAILABLE</option>
                    <option >NOT_AVAILABLE</option>
                    <option >PENDING</option>
                    <option >RESERVED</option>
                    <option >TO_CONFIRM</option>
              ...
              <div>
              <a href="managevehicle.php"><button style="margin-bottom:5px;width:150px; "  type="sumbit" name="clear" class="btn btn-primary px-4">Clear Filter</button></a>
                </div>
                 <!-- /.Filter -->
          ...
          <div class="searchbox" style="display:inline-block;float:right;">
             <input style="width:270px;" type="text" name="search" value="<?php if(isset($_GET['search'])){echo $_GET['search'];} ?>"  placeholder="Search by Vehicle Model or Name or Brand">
              ...
              <?php
              if(isset($_GET['selector']) && isset($_GET['status']) && isset($_GET['start_price']) && isset($_GET['end_price'])){
                ...
                $sql="SELECT * FROM vehicle where userid = $id AND status != 'DELETED' AND  vehicle.rate  BETWEEN $startprice AND $endprice AND vehicle.status = '".$status."' AND vehicle.vehicletype = '".$selector."' ";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
               }
                   else if(isset($_GET['start_price']) && isset($_GET['end_price']) && isset($_GET['selector'])){

                    $selector = $_GET['selector'];
                    $startprice = $_GET['start_price'];
                    $endprice = $_GET['end_price'];
                    
                    $sql="SELECT * FROM vehicle where userid = $id AND status != 'DELETED' AND  vehicle.rate  BETWEEN $startprice AND $endprice AND vehicle.vehicletype = '".$selector."'";
                    $query=$dbh->prepare($sql);
                    $query->execute();
                    $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
                  else if(isset($_GET['start_price']) && isset($_GET['end_price']) && isset($_GET['status'])){

                    $status = $_GET['status'];
                    $startprice = $_GET['start_price'];
                    $endprice = $_GET['end_price'];
                    
                    $sql="SELECT * FROM vehicle where userid = $id AND status != 'DELETED' AND  vehicle.rate  BETWEEN $startprice AND $endprice AND vehicle.status = '".$status."'";
                    $query=$dbh->prepare($sql);
                    $query->execute();
                    $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
                ...
                $sql="SELECT * FROM vehicle where userid = $id AND status != 'DELETED' AND vehicle.vehicletype = '".$selector."'";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                ...
                if($search == null)
                {
                  ...
                  $sql="SELECT * FROM vehicle where userid = $id AND status != 'DELETED' AND  (vehicle.model LIKE '%$search%' OR vehicle.brand LIKE '%$search%' OR vehicle.vehiclename LIKE '%$search%') ";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                }
                 ...
                 $sql="SELECT * FROM vehicle where userid = $id AND status != 'DELETED' AND vehicle.rate  BETWEEN $startprice AND $endprice ";
                 $query=$dbh->prepare($sql);
                 $query->execute();
                 $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
                ...
                {
                  $sql="SELECT * FROM vehicle where userid = $id AND status != 'DELETED'";
                     $query=$dbh->prepare($sql);
                     $query->execute();
                     $results=$query->fetchALL(PDO::FETCH_OBJ);
                }
 ...
 <!--cards -->
    ...
    <div class="title"> 
        <h3> <?php echo htmlentities($result->vehiclename);?> </h3>
    </div>
    <div class="des">
      <?php if(htmlentities($result->status) == 'RESERVED' || htmlentities($result->status) == 'NOT_AVAILABLE' ) {?>
        <p style="color:red">Status : <?php echo htmlentities($result->status);?></p>
        <?php }else if(htmlentities($result->status) == 'AVAILABLE' ) {?> 
        <p style="color:green">Status : <?php echo htmlentities($result->status);?></p>
        <?php }else if(htmlentities($result->status) == 'PENDING' || htmlentities($result->status) == 'TO_CONFIRM') {?> 
          <p style="color:orange">Status : <?php echo htmlentities($result->status);?></p>
        <?php }?> 
    </div>
    <?php if(htmlentities($result->status) == 'AVAILABLE' || htmlentities($result->status) == 'NOT_AVAILABLE') {?>
    <div class="btns">
    <a href="editvehicle.php?editid=<?php echo htmlentities($result->vehicleid)?>" ><button class="button2 button2-primary button2-sm"><i class="fa fa-pencil"></i></button></a>
    <a href="deletevehicle.php?deleteid=<?php echo htmlentities($result->vehicleid)?>&vehicleimage=<?php echo htmlentities($result->vehicleimage)?>" onclick="return confirm('Are you sure you want to delete this vehicle?')"><button class="button2 button2-danger button2-sm"><i class="fa fa-trash"></i></button></a>
    </div>
    <div class="feedbacklink" align="Center" style="margin-bottom:10px;">
    ...
    <div class="feedbacklink" align="Center" style="margin-bottom:30px;margin-top:45px;">
      <a href="viewvehiclefeedbacks.php?vehicleid=<?php echo htmlentities($result->vehicleid)?>">View Feedbacks</a>
      </div>
      <?php }  ?>
    ...
    <?php } else { ?>
      ...
      <div class="row" >
        <div class="col-xs-12" >
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"><i class="fa fa-cab"></i> Manage Vehicle</h3>
            ...
            <h2 style="text-align:center; margin-top:10%;"> You need to <bold>Verify<bold> your account to access this page!<h2>
            <h2 style="text-align:center"> Go to your <a href="profile.php">Profile</a> page and Upload Valid ID's.<h2>
