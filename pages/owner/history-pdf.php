...
class myPDF extends FPDF{
    ...
    function headerTable(){
        ...
        $this->Cell(30,10,'Vehicle Name',1,0,'C');
        $this->Cell(40,10,'Vehicle Model',1,0,'C');
    ...
    function viewTable($dbh){
        ...
        {
        $this->Cell(30,15,$result->vehiclename,1,0,'L');
    ...
    {
        $this->Cell(285,10,'No Data Available',1,1,'C');
    }
    }
    else{
        $sql= "SELECT * FROM bookinghistory
                 ...
                 INNER JOIN rentercredentials ON bookinghistory.renterid = rentercredentials.renterid
                 INNER JOIN user ON rentercredentials.renterid = user.userid
                 WHERE vehicle.userid = $id AND MONTH(bookinghistory.date) =  $month";
        $query=$dbh->prepare($sql);
        $query->execute();
        $results=$query->fetchALL(PDO::FETCH_OBJ);
        if ($query->rowCount()>0) {
    ...
    function Total($dbh) {
        ...
        WHERE vehicle.userid = $id";
                ...
                if ($result->totalpayment > 0) 
                {
                 ...
                 $this->Cell(255,10,number_format((float)$result->totalpayment, 2, '.', ',').''.' Pesos',1,1,'L');
                }
                else
                {
    ...
    function Penalty($dbh) {
        ...
        if($month == 'All')
        {
            ...
            WHERE vehicle.userid = $id";
                ...
                {
                 $this->SetFont('Times','B',12);
                 $this->Cell(30,10, 'Total Penalty:',1,0,'C'); 
                 $this->Cell(255,10,number_format((float)$result->totalpenalty, 2, '.', ',').''.' Pesos',1,1,'L');
                }
                else
                {
