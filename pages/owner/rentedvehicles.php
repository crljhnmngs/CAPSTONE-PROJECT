...
<head>
  ...
  <title>Vehicle Owner - Reserved Vehicles</title>
...
<?php  include('D:\PROGRAMMING SOFTWARES\XAMPP\htdocs\WeLift\includes\ownerheader.php');      ?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1 align="center">
        <i class="fa fa-car"></i> View Reserved Vehicles
        <small></small>
      </h1>
    ...
    <?php
               ...
               INNER JOIN booking ON vehicle.vehicleid = booking.vehicleid
               WHERE userid = $id AND booking.bookingstatus = 'RESERVED'";
               $query=$dbh->prepare($sql);
               $query->execute();
               $results=$query->fetchALL(PDO::FETCH_OBJ);
        ...
        <div class="col-xs-12" >
          ...
          <div class="searchbox" style="display:inline-block;float:right;">
             <input style="width:270px;" type="text" name="search" value="<?php if(isset($_GET['search'])){echo $_GET['search'];} ?>"  placeholder="Search by Vehicle Model or Name or Brand">
              ...
              WHERE vehicle.userid = $id AND booking.bookingstatus =  'RESERVED' AND  vehicle.rate  BETWEEN $startprice AND $endprice AND vehicle.vehicletype = '".$selector."'";
                    $query=$dbh->prepare($sql);
                    $query->execute();
                    $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
                ...
                INNER JOIN rentercredentials ON booking.renterid = rentercredentials.renterid
                INNER JOIN user ON rentercredentials.renterid = user.userid
                WHERE vehicle.userid = $id AND booking.bookingstatus =  'RESERVED' AND vehicle.vehicletype = '".$selector."'";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
              ...
              INNER JOIN rentercredentials ON booking.renterid = rentercredentials.renterid
              INNER JOIN user ON rentercredentials.renterid = user.userid
              WHERE vehicle.userid = $id AND booking.bookingstatus =  'RESERVED' AND  (vehicle.model LIKE '%$search%' OR vehicle.brand LIKE '%$search%' OR vehicle.vehiclename LIKE '%$search%') ";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                }
              ...
              INNER JOIN rentercredentials ON booking.renterid = rentercredentials.renterid
              INNER JOIN user ON rentercredentials.renterid = user.userid
              WHERE vehicle.userid = $id AND booking.bookingstatus =  'RESERVED' AND vehicle.rate  BETWEEN $startprice AND $endprice ";
                 $query=$dbh->prepare($sql);
                 $query->execute();
                 $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
 ...
 <!--cards -->
    ...
    <?php include('rentedvehicleinfo.php'); ?>              
<!--cards -->
