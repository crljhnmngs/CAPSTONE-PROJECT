...
if(!isset($_SESSION['userid']))
  {
    ...
    //$currentdatetime = strtotime(date('2022-05-19 09:45:00'));
...
<head>
  ...
  <title>Vehicle Owner - Vehicle Booking</title>
...
<?php  include('D:\PROGRAMMING SOFTWARES\XAMPP\htdocs\WeLift\includes\ownerheader.php');      ?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1 align="center">
        <i class="fa fa-car"></i> Manage Vehicle Bookings
        <small></small>
      </h1>
    ...
    <?php
               ...
               INNER JOIN vehicle ON booking.vehicleid = vehicle.vehicleid  where vehicle.userid = $id AND bookingstatus = 'PENDING'";
               $query=$dbh->prepare($sql);
               $query->execute();
               $results=$query->fetchALL(PDO::FETCH_OBJ);
    ...
    <section class="content" >
      <div class="row" >
        <div class="col-xs-12" >
          ...
          <div class="box" style="min-height: 77vh;" >
            <div class="box-header">
              <h3 class="box-title"><i class="fa fa-cab"></i> Vehicle Bookings </h3>
            </div>
            <!-- /.box-header -->
           ...
           <label class="box-title" style="margin-top:10px;"> Filter by vehicle type: </label>
            ...
            <option value="" disabled selected="" hidden >Select Filter</option>
              ...
              <div>
              <a href="vehiclerental.php"><button style="margin-bottom:5px;width:150px; "  type="sumbit" name="clear" class="btn btn-primary px-4">Clear Filter</button></a>
                </div>
                 <!-- /.Filter -->
          ...
          <div class="caution" style="height:22px;width:22px;float:right;margin-top:60px;margin-right:20px;">
          ...
          <div class="hide1" style="float:right;margin-top:80px;">Vehicle Booking will automatically cancelled after 6 hours.</div>
              ...
              <?php
                   if(isset($_GET['start_price']) && isset($_GET['end_price']) && isset($_GET['selector'])){
                    ...
                    WHERE vehicle.userid = $id AND booking.bookingstatus =  'PENDING' AND  vehicle.rate  BETWEEN $startprice AND $endprice AND vehicle.vehicletype = '".$selector."'";
                    $query=$dbh->prepare($sql);
                    $query->execute();
                    $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
                ...
                $sql= "SELECT * FROM booking 
                  ...
                  INNER JOIN rentercredentials ON booking.renterid = rentercredentials.renterid
                  INNER JOIN user ON rentercredentials.renterid = user.userid
                  WHERE vehicle.userid = $id AND booking.bookingstatus =  'PENDING' AND vehicle.vehicletype = '".$selector."'";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                ...
                if($search == null)
                {
                  ...
                  INNER JOIN rentercredentials ON booking.renterid = rentercredentials.renterid
                  INNER JOIN user ON rentercredentials.renterid = user.userid
                  WHERE vehicle.userid = $id AND booking.bookingstatus =  'PENDING' AND  (vehicle.model LIKE '%$search%' OR vehicle.brand LIKE '%$search%' OR vehicle.vehiclename LIKE '%$search%') ";
                $query=$dbh->prepare($sql);
                $query->execute();
                $results=$query->fetchALL(PDO::FETCH_OBJ);
                }
                  ...
                  else if($endprice < $startprice )
                  {
                    ...
                    INNER JOIN rentercredentials ON booking.renterid = rentercredentials.renterid
                    INNER JOIN user ON rentercredentials.renterid = user.userid
                    WHERE vehicle.userid = $id AND booking.bookingstatus =  'PENDING' AND vehicle.rate  BETWEEN $startprice AND $endprice ";
                 $query=$dbh->prepare($sql);
                 $query->execute();
                 $results=$query->fetchALL(PDO::FETCH_OBJ);
                  }
                ...
                else
                {
                  ...
                  INNER JOIN rentercredentials ON booking.renterid = rentercredentials.renterid
                  INNER JOIN user ON rentercredentials.renterid = user.userid
                  WHERE vehicle.userid = $id AND booking.bookingstatus =  'PENDING'";
                     $query=$dbh->prepare($sql);
                     $query->execute();
                     $results=$query->fetchALL(PDO::FETCH_OBJ);
                }
                ...
                if ($query->rowCount()<=0) {
                  ...
                  {
                    $vehicleid = htmlentities($result->vehicleid);
                    $bookingdate = htmlentities($result->bookingdate);
