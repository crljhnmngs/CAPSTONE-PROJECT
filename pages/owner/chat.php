...
<head>
  ...
  <title>Vehicle Owner - Chat</title>
                ...
                <div class="col-md-4 text-light" style="height:550px; width: 350px; ">
                    ...
                    <div class="row justify-content-center align-items-center pb-4 d-flex">
                        <div class="col-12 input-group-sm px-4">
                            <input class="form-control rounded-pill shadow-none" type="text" placeholder="Search Client" aria-describedby="inputGroup-sizing-sm">
                        </div>
                    </div> 
                        <?php
                            $sql="SELECT * FROM message
                            INNER JOIN rentercredentials ON message.receiverid = rentercredentials.renterid OR message.senderid = rentercredentials.renterid
                            INNER JOIN user ON rentercredentials.renterid = user.userid WHERE message.senderid = $id OR message.receiverid = $id GROUP BY rentercredentials.renterid";
                        ...
                        <div class="row px-2">
                            <button type="submit" name="submit" value="submit" class="btn btn-warning text-white shadow-none">
                                <div class="row py-2 px-2">
                                    <div class="col-md-2">
                                    <img src="../../dist/img/userimage/<?php echo htmlentities($result->image);?>" class="img-circle" alt="User Image" style="height:50px;">
                                    </div>
                                    <input type="hidden" name="receiverid" value="<?php echo htmlentities($result->renterid)?>">
                            ...
                            <div class="col-md-9">
                                <?php
                                    if(isset($_POST['submit'])){
                                        ...
                                        $sql1="SELECT * from rentercredentials 
                                        INNER JOIN user ON rentercredentials.renterid = user.userid
                                        WHERE rentercredentials.renterid=$receiverid";
                        ...
                        <div class="col-sm-12 chatBox" style="height: 450px; overflow-y: auto; ">
                                <!-- <?php
                                 if(isset($_POST['submit'])){
                                    ...
                                    <input type="text" required name="receiverid" value="<?php echo $row['renterid']?>">
                                    
                                    <?php
                                    if(isset($row['receiverid'])){
                                        $senderid = mysqli_real_escape_string($connection, $_SESSION['userid']);
                                        $receiverid = mysqli_real_escape_string($connection, $_POST['renterid']);
                        ...
                        <div class="row pt-2" style="background: #302D32">
                            <form class="typing-area">
                                <div class="input-group pb-2">
                                    ...
                                    <button class="btn1 fa-solid fa-paper-plane col-sm-1" type="submit" name="send" style="color: #F8F8F8; border: none; background-color: #302D32; margin-top:15px; float:left; margin-left:-10px"></button>
                                </div>
                            </form>
                        </div>
                    </div>
...
<script src="../../dist/js/owner-chat.js"></script>  
