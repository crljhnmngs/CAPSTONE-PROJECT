...
if(!isset($_SESSION['userid']))
 ...
 else if(!isset($_GET['complainantid']))
  {
  ...
  if (isset($_POST['submit'])) {
    ...
    $complainantid=intval($_GET['complainantid']);
    $reportid=intval($_GET['reportid']);
    $message1=($_POST['message']);
    $date = date('y-m-d H:i:s');
    $email=($_GET['email']);

    $sql = "UPDATE complaints SET status='REPLIED' WHERE reportid = $reportid ";
    $query = $dbh->prepare($sql);
    $query->execute();

    $sql3 = "INSERT INTO notification(receiverid,message,header,date)VALUES($complainantid,'WeLift Admin replied your report, Please Check your email.','email', '$date')";
    ...
    echo "<script type ='text/javascript'> document.location.href='report.php' </script>"; 
...
<head>
  ...
  <title>Admin - Reply Complainant</title>
...
<?php  include('D:\PROGRAMMING SOFTWARES\XAMPP\htdocs\WeLift\includes\renterheader.php');      ?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1 align="center">
        <i class="fa fa-comment"></i> Reply Complainant
        <small></small>
      </h1>
    </section>
    ...
    <div class="rate" align="center">
        <h5 style="margin-top:0px; padding-top:50px;"> Reply  </h5>
    </div>

    

    <div class="comment"  align="center" style="margin-top:20px; ">
    <textarea placeholder="Your message.." name="message" cols="50" rows="10" required></textarea>
    </div>
